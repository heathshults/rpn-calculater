...
*
*/
	function rpnCalculator({ debug, help, fullStack, object:{} }: optionProps) {
	 ...
	 * 
	 */
	function rpn({ opers, options = {} }: { opers; options?: {}; }) {
		let calc: any = rpnCalculator(options);
...
*
*/
	function rpnCalculatorFactory(opers,options): any{
	...
	if (module.parent) {
		...
		(function main() {
			...
			let result = rpn({ opers: cli.parms, options: cli.opts });
